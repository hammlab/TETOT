...
def aug(x_in):
    #x_in = tf.image.pad_to_bounding_box(x_in, 4, 4, HEIGHT + 8, WIDTH + 8)
    #x_in = tf.image.random_crop(x_in, [len(x_in), HEIGHT, WIDTH, NCH])
...
def load_VLCS_source_TT(sources):
    ...
    for i, environment in enumerate(environments):
        ...
        if i in sources:
            ...
            #train set
            train_loader = DataLoader(env_dataset_2, len(env_dataset_2))
            ...
            print("Labels:", np.unique(train_dataset_labels_array))
            
            source_train_data.append(train_dataset_array)
            source_train_labels.append(train_dataset_labels_array)
            
            #test set
            test_loader = DataLoader(env_dataset_1, len(env_dataset_1))
            test_dataset_array = next(iter(test_loader))[0].permute(0, 2, 3, 1).numpy()
            test_dataset_labels_array = next(iter(test_loader))[1].numpy()
            print("Labels:", np.unique(test_dataset_labels_array))
            
            source_test_data.append(test_dataset_array)
            source_test_labels.append(test_dataset_labels_array)

    return source_train_data, source_train_labels, source_test_data, source_test_labels

def eval_entropy_disc(x_test, classifier):
    points = 0
    entropy = 0
    batch_size = 50
...
def eval_accuracy_bec(x_test, y_test, base_model, encoder, classifier):
    correct = 0
    points = 0
    loss = 0
    batch_size = 50
